...
//        doNothing().when(databaseConnectionManager).init();
    ...
    public void testConcurrentRSVPApplications() throws InterruptedException {
        int numberOfThreads = 30;  // 例如模拟 10 个用户并发申请
        int eventId = 4;  // 测试的 eventId
        int studentId = 3;  // 假设一个学生 ID
        int numTickets = 2;  // 每个学生申请 2 张票
        List<Integer> participates_id = Arrays.asList(3, 4);  // 参与者 ID 列表
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            Callable<Boolean> task = () -> {
                try {
                    // 调用 applyForRSVP 方法
                    eventService.applyForRSVP(eventId, currentStudentId, numTickets, participates_id);
                    return true;  // 成功
                } catch (Exception e) {
